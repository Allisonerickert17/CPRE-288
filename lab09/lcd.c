 ...
 * Updated on 8/22/19 for compatibility with Isaac Rex's timer fixes.  (date
 * corrected by phjones)
 *
 * 
 *
 *
...
uint8_t lcd_reverseNibble(uint8_t x) {
  return (((x & 0b0001) << 3) | ((x & 0b0010) << 1) | ((x & 0b0100) >> 1) |
          ((x & 0b1000) >> 3));
}
...
void inline lcd_home(void) { lcd_sendCommand(HD_RETURN_HOME); }
...
/**
 * Mimics the C library function printf for writing to the LCD screen.  The
 * function is buffered; i.e. if you call lprintf twice with the same string, it
 * will only update the LCD the first time.
 *
 * Google "printf" for documentation on the formatter string.
 *
 * Code from this site was also used:
...
void lcd_printf(const char *format, ...) {
  ...
  while (*str && charnum < LCD_TOTAL_CHARS) {
    ...
    /*
     * The LCD's lines are not sequential; for future reference, the address are
     * like 0x00...0x13 : line 1 0x14...0x27 : line 3 0x28...0x3F : random junk
     * 0x40...0x53 : line 2
     * 0x54...0x68 : line 4
     *
     * The cursor position must be reset at the end of every line, otherwise,
     * after writing line 1, it writes line 3 and then nothingness
     */
